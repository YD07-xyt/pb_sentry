...
public:
  ...
  double RadiusOutlier_radius;
  int RadiusOutlier_Point_nums;
    ...
    : Node("ground_segmentation", node_options)
{
  ...
  RadiusOutlier_radius = this->declare_parameter("RadiusOutlier_radius", RadiusOutlier_radius);
  RadiusOutlier_Point_nums = this->declare_parameter("RadiusOutlier_Point_nums", RadiusOutlier_Point_nums);
    ...
    const sensor_msgs::msg::PointCloud2::SharedPtr msg)
{
  ...
  outrem.setInputCloud(obstacle_cloud.makeShared());       // 设置输入点云
  outrem.setRadiusSearch(RadiusOutlier_radius);      // 设置半径范围，例如8厘米
  outrem.setMinNeighborsInRadius(RadiusOutlier_Point_nums); // 设置半径范围内最少点数，低于这个点数的点被删掉
  outrem.setKeepOrganized(false);    // 设置是否保持点云的组织结构
  ...
  pcl::toROSMsg(*cloud_filtered, *obstacle_msg);
