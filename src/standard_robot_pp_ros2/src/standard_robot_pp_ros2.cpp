...
StandardRobotPpRos2Node::StandardRobotPpRos2Node(
  ...
  // //   std::bind(&StandardRobotPpRos2Node::feedback_callback, this,
  // std::placeholders::_1, std::placeholders::_2); timer_ =
  // create_wall_timer(100ms,
  // std::bind(&StandardRobotPpRos2Node::action_client_callback, this));
  // //#########################################################################
...
void StandardRobotPpRos2Node::createPublisher() {
  ...
  robot_state_info_pub_ =
      this->create_publisher<pb_rm_interfaces::msg::RobotStateInfo>(
          "serial/robot_state_info", 10);
  joint_state_pub_ = this->create_publisher<sensor_msgs::msg::JointState>(
      "serial/gimbal_joint_state", 10);
  robot_motion_pub_ = this->create_publisher<geometry_msgs::msg::Twist>(
      "serial/robot_motion", 10);
  event_data_pub_ = this->create_publisher<pb_rm_interfaces::msg::EventData>(
      "referee/event_data", 10);
  all_robot_hp_pub_ =
      this->create_publisher<pb_rm_interfaces::msg::GameRobotHP>(
  ...
  ground_robot_position_pub_ =
      this->create_publisher<pb_rm_interfaces::msg::GroundRobotPosition>(
  ...
  buff_pub_ =
      this->create_publisher<pb_rm_interfaces::msg::Buff>("referee/buff", 10);
}
...
void StandardRobotPpRos2Node::createNewDebugPublisher(const std::string &name) {
  ...
  auto debug_pub =
      this->create_publisher<example_interfaces::msg::Float64>(topic_name, 10);
...
void StandardRobotPpRos2Node::createSubscription() {
  cmd_vel_sub_ = this->create_subscription<geometry_msgs::msg::Twist>(
      "cmd_vel", 10,
      std::bind(&StandardRobotPpRos2Node::cmdVelCallback, this,
                std::placeholders::_1));

  cmd_gimbal_joint_sub_ =
      this->create_subscription<sensor_msgs::msg::JointState>(
          "cmd_gimbal_joint", 10,
          std::bind(&StandardRobotPpRos2Node::cmdGimbalJointCallback, this,
                    std::placeholders::_1));
  ...
  cmd_tracking_sub_ =
      this->create_subscription<auto_aim_interfaces::msg::Target>(
          "tracker/target", 10,
          std::bind(&StandardRobotPpRos2Node::visionTargetCallback, this,
                    std::placeholders::_1));

  goal_state_sub_ = this->create_subscription<pb_rm_interfaces::msg::NavGoal>(
      "goal_state_topic", 10,
      std::bind(&StandardRobotPpRos2Node::getGoalState, this,
                std::placeholders::_1));

  action_status_sub_ =
      this->create_subscription<action_msgs::msg::GoalStatusArray>(
          "/navigate_to_pose/_action/status", 10,
          std::bind(&StandardRobotPpRos2Node::actionStatusCallback, this,
                    std::placeholders::_1));
}
...
void StandardRobotPpRos2Node::getParams() {
  ...
  try {
    is_two_serial = declare_parameter("is_two_serial", false);
...
void StandardRobotPpRos2Node::serialPortProtect() {
  ...
  try {
    if (!serial_driver_->port()->is_open()) {
      ...
      RCLCPP_INFO(get_logger(), "Serial 1 port opened!");
      is_usb_ok_ = true;
    }
  } catch (const std::exception &ex) {
    RCLCPP_ERROR(get_logger(), "Open serial 1 port failed : %s", ex.what());
    is_usb_ok_ = false;
  }
  try {
    if (!serial_driver2_->port()->is_open()) {
      serial_driver2_->port()->open();
      RCLCPP_INFO(get_logger(), "Serial 2 port opened!");
      is_usb_ok2_ = true;
    }
  } catch (const std::exception &ex) {
    RCLCPP_ERROR(get_logger(), "Open serial 2 port failed : %s", ex.what());
    is_usb_ok2_ = false;
  }
...
void StandardRobotPpRos2Node::receiveData() {
  ...
  while (rclcpp::ok()) {
    if (!is_usb_ok_) {
      RCLCPP_WARN(get_logger(), "receive: usb is not ok! Retry count: %d",
                  retry_count++);
      std::this_thread::sleep_for(
    ...
    try {
      ...
      if (sof[0] != SOF_RECEIVE) {
        sof_count++;
        // RCLCPP_WARN(get_logger(), "未匹配到 SOF! 收到的字节: 0x%02X, 期望:
        // 0x%02X, 累计丢弃: %d",
      ...
      //    data_buf.resize(correct_size); //
      //    强制裁掉后面多出来的“第二个包”或者“0字节”
      //  }
...
void StandardRobotPpRos2Node::publishDebugData(
  ...
  static rclcpp::Publisher<example_interfaces::msg::Float64>::SharedPtr
      debug_pub;
...
void StandardRobotPpRos2Node::publishImuData(ReceiveImuData &imu_data) {
  ...
  RCLCPP_INFO(get_logger(), "imu_msg:%f,%f,%f", imu_data.data.roll_vel,
              imu_data.data.pitch_vel, imu_data.data.yaw_vel);
  joint_msg.name = {
      "gimbal_pitch_joint",
      "gimbal_yaw_joint",
...
void StandardRobotPpRos2Node::publishRobotInfo(
  ...
  msg.models.custom_controller = robot_models_.custom_controller.at(
...
void StandardRobotPpRos2Node::publishAllRobotHp(
...
void StandardRobotPpRos2Node::publishGameStatus(
  ...
  if (record_rosbag_ &&
    ...
    if (!callTriggerService(service_name)) {
      RCLCPP_ERROR(get_logger(), "Failed to call service: %s",
                   service_name.c_str());
    }
  }
}

void StandardRobotPpRos2Node::publishRobotMotion(
...
void StandardRobotPpRos2Node::publishRfidStatus(
    ReceiveRfidStatus &rfid_status) {
  pb_rm_interfaces::msg::RfidStatus msg;

  msg.base_gain_point = rfid_status.data.base_gain_point;
  msg.central_highland_gain_point =
      rfid_status.data.central_highland_gain_point;
  msg.enemy_central_highland_gain_point =
      rfid_status.data.enemy_central_highland_gain_point;
  ...
  msg.friendly_fly_ramp_front_gain_point =
      rfid_status.data.friendly_fly_ramp_front_gain_point;
  msg.friendly_fly_ramp_back_gain_point =
      rfid_status.data.friendly_fly_ramp_back_gain_point;
  msg.enemy_fly_ramp_front_gain_point =
      rfid_status.data.enemy_fly_ramp_front_gain_point;
  msg.enemy_fly_ramp_back_gain_point =
      rfid_status.data.enemy_fly_ramp_back_gain_point;
  ...
  msg.friendly_highway_lower_gain_point =
      rfid_status.data.friendly_highway_lower_gain_point;
  msg.friendly_highway_upper_gain_point =
      rfid_status.data.friendly_highway_upper_gain_point;
  msg.enemy_highway_lower_gain_point =
      rfid_status.data.enemy_highway_lower_gain_point;
  msg.enemy_highway_upper_gain_point =
      rfid_status.data.enemy_highway_upper_gain_point;
  msg.friendly_fortress_gain_point =
      rfid_status.data.friendly_fortress_gain_point;
  msg.friendly_outpost_gain_point =
      rfid_status.data.friendly_outpost_gain_point;
  msg.friendly_supply_zone_non_exchange =
      rfid_status.data.friendly_supply_zone_non_exchange;
  msg.friendly_supply_zone_exchange =
      rfid_status.data.friendly_supply_zone_exchange;
  msg.friendly_big_resource_island =
      rfid_status.data.friendly_big_resource_island;
...
void StandardRobotPpRos2Node::sendData() {
  ...
  while (rclcpp::ok()) {
    if (is_two_serial) {
      if (!is_usb_ok2_) {
        RCLCPP_WARN(get_logger(), "send: usb 2 is not ok! Retry count: %d",
                    retry_count++);
        std::this_thread::sleep_for(
            std::chrono::milliseconds(USB_NOT_OK_SLEEP_TIME));
        continue;
      ...
      if (!is_usb_ok_) {
        RCLCPP_WARN(get_logger(), "send: usb is not ok! Retry count: %d",
                    retry_count++);
        std::this_thread::sleep_for(
            std::chrono::milliseconds(USB_NOT_OK_SLEEP_TIME));
        continue;
...
void StandardRobotPpRos2Node::action_client_callback() {
  ...
  send_goal_options.result_callback = std::bind(
      &StandardRobotPpRos2Node::result_callback, this, std::placeholders::_1);
  this->goal_client_ptr_->async_send_goal(goal_msg, send_goal_options);
}
void StandardRobotPpRos2Node::result_callback(
...
void StandardRobotPpRos2Node::cmdGimbalJointCallback(
  ...
  if (msg->name.size() != msg->position.size()) {
    RCLCPP_ERROR(
        get_logger(),
        "JointState message name and position arrays are of different sizes");
    return;
  }
...
void StandardRobotPpRos2Node::cmdShootCallback(
...
void StandardRobotPpRos2Node::setParam(const rclcpp::Parameter &param) {
  if (!initial_set_param_) {
    ...
    std::vector<std::string> possible_detectors = {"armor_detector_openvino",
    ...
    if (detector_node_name_.empty()) {
      RCLCPP_WARN_THROTTLE(get_logger(), *this->get_clock(), 1000,
                           "No detector node found!");
      return;
    }

    detector_param_client_ = std::make_shared<rclcpp::AsyncParametersClient>(
        this, detector_node_name_);
    if (!detector_param_client_->service_is_ready()) {
      RCLCPP_WARN_THROTTLE(get_logger(), *this->get_clock(), 1000,
                           "Service not ready, skipping parameter set");
      return;
    }
  }
...
bool StandardRobotPpRos2Node::callTriggerService(
  ...
  while (!client->wait_for_service(0.1s)) {
    if (!rclcpp::ok()) {
      RCLCPP_ERROR(get_logger(),
                   "Interrupted while waiting for the service: %s",
                   service_name.c_str());
      return false;
    }
    ...
    if (elapsed_time > std::chrono::seconds(5)) {
      RCLCPP_ERROR(get_logger(),
                   "Service %s not available after 5 seconds, giving up.",
                   service_name.c_str());
      return false;
    }
  ...
  if (rclcpp::spin_until_future_complete(this->shared_from_this(), result) ==
    ...
    RCLCPP_INFO(get_logger(), "Service %s call succeeded: %s",
                service_name.c_str(), result.get()->success ? "true" : "false");
