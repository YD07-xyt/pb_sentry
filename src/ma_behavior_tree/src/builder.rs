...
use error::{CreateNodeError, CreatePubSubError, MissingArgumentError};
...
#[extension(pub trait ServiceBuilder)]
impl ros2_client::Node {
    fn pub_sub_builder<M>(&mut self) -> Ros2PubSubBuilder<'_, M> {
        Ros2PubSubBuilder::new().node(self)
    }

    fn action_client_builder<A>(&mut self) -> Ros2ActionClientBuilder<'_, A> {
